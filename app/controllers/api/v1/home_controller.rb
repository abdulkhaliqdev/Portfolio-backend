class Api::V1::HomeController < Api::BaseController
  def index
    @projects = Project.eager_load(:image_attachment).all
    @articles = Article.eager_load(:main_image_attachment, :image_gallery_attachments).all
    @user     = User.eager_load(:prime_image_attachment, :secondary_image_attachment, :third_image_attachment).all
